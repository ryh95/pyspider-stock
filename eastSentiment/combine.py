...
def getPriceAndSentimentFactor(stockcode):
    client = MongoClient()
    ...
    dates = []
    # 这是将股票时间比对舆论时间，找到既有股价又有舆论的那一天，然后将日期|舆论因子的自然对数放入data，后面画出图像日期舆论图像
    ...
    # 将既有股价又有舆论的那一天的日期、情感因子、股价插入mongodb，collection命名为priceAndsentimentFactor

    for i in range(len(dates)):
        result = db.priceAndsentimentFactor.insert_one({
            "date": dates[i],
            "sentiment_factor": y[i],
            "price": z[i]
        })
